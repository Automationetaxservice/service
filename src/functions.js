...
function accounts(account){
  ...
  conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', function(err, res) {
    conn.query("SELECT Id, Name FROM Account WHERE Name LIKE '%"+account+"%' ORDER By Name", function(err, result) {
      var elemento = "";
...
function contacts(contact){
  ...
  var conn = new conexionSF();
  conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', function(err, res) {
    conn.query("SELECT Id, Name FROM Contact WHERE Name LIKE '%"+contact+"%' ORDER By Name", function(err, result) {
      var elemento = "";
      for(var k = 0; k < result.totalSize; k++){
...
function keywords(){
  ...
  var conn = new conexionSF();
  conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', function(err, res) {
    conn.query("SELECT Id, Name, Case_Resolution__c FROM Keywords_Cases__c WHERE Name LIKE '%"+key+"%' ORDER BY Name", function(err, result) {
      var elemento = "";
      for(var k = 0; k < result.totalSize; k++){
...
var nextScript = function nextScript(){
  ...
  if(child.id == "modalScriptAuthentication"){
    ...
    if(true){
      ...
      var conn = new conexionSF();
      conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', function(err, res) {
        conn.query("SELECT Id FROM Case WHERE Customer_Support__c ='"+idCService+"' and AccountId='"+accountId+"' and ContactId='"+contactId+"' ", function(err, result) {
  ...
  if(child.id == "modalScriptAgreement"){
    ...
    for (var j=0; j<apoyarImpuestos.length; j++) {  if(apoyarImpuestos[j].checked && apoyarImpuestos[j].value !== "All") { count4++; if(count4 == total4){ checkapoyarImpuestos += apoyarImpuestos[j].value; }else{ checkapoyarImpuestos += apoyarImpuestos[j].value + ';'; } } }
  ...
  if(child.id == "modalScriptRecap"){
    ...
    if(!document.getElementById("cuantosDependientesRecap").value.trim() || document.getElementById("cantidadDineroRecap").value == "0"){ document.getElementById("messageWarning").innerHTML = "<span class='warningMessage'>Introduzca un valor mayor a 0 en el punto 4.3.1</span>"; document.getElementById("messageWarning").classList.remove("hidden"); setTimeout(function() { document.getElementById("messageWarning").classList.add("hidden"); }, 2000); return false; }
...
function newCase(){
  ...
  var conn = new conexionSF();
  conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', async function(err, res) {
    conn.query("SELECT Id FROM Case WHERE Customer_Support__c ='"+idCS+"' and AccountId='"+accountId+"' and ContactId='"+contactId+"' ", async function(err, result) {
      ...
      if(result.totalSize == 0){
        //Crear Caso
          ...
          try{
            ...
            if(caseRes){
              ...
              const caso = await conn.sobject("Case").create(
                {
                  AccountId: accountId,
                  ContactId: contactId,
              ...
              //Crear Oportunidad
              const ret = await conn.sobject("Opportunity").create({
              ...
              //Crear Script Case
              const scase = await conn.sobject("Script_Case__c").create(
                {
        ...
        //Actualizar Keyword del Caso
          ...
          if(caseRes){
            ...
            conn.query("SELECT Id FROM Script_Case__c WHERE Account__c='"+accountId+"' and Contact__c='"+contactId+"' and Customer_Support__c='"+idCS+"' ", async function(err, result) {
              const sc = await conn.sobject("Script_Case__c").update({
            ...
            conn.query("SELECT Id FROM Case WHERE AccountId='"+accountId+"' and ContactId='"+contactId+"' and Customer_Support__c='"+idCS+"' ", async function(err, result) {
            ...
            conn.query("SELECT Id FROM Opportunity WHERE AccountId='"+accountId+"' and Contact__c='"+contactId+"' and Customer_Support__c='"+idCS+"' ", async function(err, result) {
              const opp = await conn.sobject("Opportunity").update(
                {
                  Id: result.records[0].Id,
                  Name: name+', '+checkapoyarImpuestos.split(";").pop()+', '+contactName
                }
      ...
      //Revisar cuándo se crea el evento
      /*
      //Delete
      conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        const caso = await conn.sobject("Case").delete('5005f00000qfIQkAAM');
        const oportunidad = await conn.sobject("Opportunity").delete('0065f00000UFO1pAAH');
        const scase2 = await conn.sobject("Script_Case__c").delete('a3p5f000001DuFPAA0');
      });
      */
    ...
    _Lista.calendarEvents(function(eventsSF){
      if(contador == "0053k00000AvrPvAAJ"){ //Contadora Daniela
        ...
        if(prioridad == "Low-Mid"){ 
          ...
          var virtualEvents = eventsSF.filter(obj => { return obj.extendedProps.tipo2 == "Low-Mid_Virtual" });
        ...
        if(prioridad == "Monthly-Clients"){ 
          ...
          var virtualEvents = eventsSF.filter(obj => { return obj.extendedProps.tipo2 === "Monthly-Clients_Virtual" });
