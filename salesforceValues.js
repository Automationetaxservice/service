...
function accountsUser(success){
    ...
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', function(err, res) {
        conn.query("SELECT Owner_User__c FROM Account GROUP BY Owner_User__c", async function(err, result) {
            var accounts = [];
            var accountArray;
...
function keywords(success){
    var conn = new conexionSF();
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', function(err, res) {
        conn.query("SELECT Id, Name, Case_Resolution__c FROM Keywords_Cases__c ORDER BY Name", function(err, result) {
            var keywords = [];
            var keyArray;
            for(var k = 0; k < result.totalSize; k++){
...
function verRecap(id){
    ...
    var conn = new conexionSF();
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', function(err, res) {
        conn.query("SELECT Id, Hola_buen_dia_se_comunica_a_Francis_Tax__c, Account_2__c, Tipo_de_Llamada__c, Contact_2__c, Cual_fue_el_ultimo_ano_que_le_ayudamos_c__c, Tipo_de_Parentezco__c, Que_ano_le_ayudamos_con_impuestos__c, Tipo_de_Agencia__c, Fast_Note__c FROM Customer_Service__c WHERE Id='"+id+"'", async function(err, result) {
            for(var k = 0; k < result.totalSize; k++){
                //Recap Textos
...
function recapUpdate(){
    ...
    for (var j=0; j<asistidoImpuestos.length; j++) { if(asistidoImpuestos[j].checked && asistidoImpuestos[j].value !== "All") { count3++; if(count3 == total3){ checkAsistidoImpuestos += asistidoImpuestos[j].value; }else{ checkAsistidoImpuestos += asistidoImpuestos[j].value + ';'; } } }

    var conn = new conexionSF();
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        ...
        conn.query("SELECT Id, Keywords_Cases__c, X4_1_Va_a_agregar_o_retirar_algun_depen__c, X1_En_apoyo_a_su_servicio_de_declaraci__c, X2_Cu_l_ser_a_el_mejor_horario_para_comu__c, X3_Quiere_realizar_alguna_actualizaci_n__c, X4_Cambio_sus_nombres_legalmente_en_el_a__c, X21_Cambio_de_seguro_social__c, X5_0_Estado_Civil__c, X6_Tiene_su_certificado_de_matrimonio__c, Hay_ingresos_que_su_esposo_a_reportara__c, X3_2_1_Nota_que_quiere_compartir_sobre_i__c, X7_Tiene_dependes_que_declara_en_sus_new__c, X8_4_Preguntarias_Nombre__c, X8_6_Va_a_retirar_algun_dependiente_new__c, X8_Va_a_agregar_dependes__c, X8_7_Nombre_Completo_Depende_a_Retirar__c, X8_7_Menciono_que_retira_un_dependiente__c, X8_3_Alguno_de_sus_dependientes_trabaja__c, X8_5_Cantidad_de_dinero__c, X9_Va_a_asistir_con_alguien_mas_que_nece__c, X23_Pago_cuidado_infantil_para_alguno_de__c, X24_Alguno_de_sus_dependientes_esta_en_l__c, X25_Gasto_en_utiles_escolares_de_alguno__c, X28_Alguno_de_sus_dependientes_tuvo_gast__c, X11_Ya_tiene_todos_los_documentos__c, X12_Ya_tiene_sus_formas_w2new__c, X28_Ademas_de_sus_formas_W2_usted_o_su_p__c, X12_3_Cuantas_W_2_tiene__c, X12_5_Cuantas_1099_tiene_o_el_reporte_de__c, X12_7_Recibio_Desempleo__c, X19_Tiene_algun_tipo_de_ingreso_por_jueg__c, X12_8_Tiene_algun_otro_tipo_de_ingreso__c, X12_9_Anotar_otro_tipo_de_formas_de_ingr__c, X30_Enviara_algun_documento_para_realiza__c, X31_Notas_Entrevistador__c FROM Script_Case__c WHERE Id='"+id+"'", function(err, result) {
            for(var k = 0; k < result.totalSize; k++){
                ...
                if(result.records[k].X2_Cu_l_ser_a_el_mejor_horario_para_comu__c){
                    var horario = new Date(result.records[k].X2_Cu_l_ser_a_el_mejor_horario_para_comu__c);
                }else{
...
function verCaso(csId, contactId, accountId){
    var conn = new conexionSF();
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', async function(err, res) {
        conn.query("SELECT Id, CaseNumber, CreatedDate, Subject FROM Case WHERE Customer_Support__c='"+csId+"' and ContactId='"+contactId+"' and AccountId='"+accountId+"' ", function(err, result) {
            for(var k = 0; k < result.totalSize; k++){
...
function verAuthentication(){
    ...
    var urlId = url.searchParams.get("Id");
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16', async function(err, res) {
        conn.query("SELECT Id, FirstName, LastName, Phone, Email, Taxpayer_SSN__c, DOB__c, Nota_Phone__c FROM Contact WHERE Id='0033k00003CparmAAB'", function(err, result) {
        ...
        conn.query("SELECT Street__c, City__c, State__c, Country__c, Postal_Code__c FROM Address__c WHERE Contact__c='0033k00003CparmAAB'", function(err, result){
...
function scriptUpdate(){
    ...
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
            ...
            try{
                ...
                if(caseRes){
                    const cs = await conn.sobject("Customer_Service__c").update({
                        Id: document.getElementById("csList").value,
...
function authenticationUpdate(idContact, firstName, lastName, phone, phoneNotes, email, ssn, street, city, state, country, postal_code, dob){
    ...
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        ...
        conn.query("SELECT Id FROM Address__c WHERE Contact__c='"+idContact+"'", async function(err, resAddress){
            const address = await conn.sobject("Address__c").update({
...
function filingDivProbe(){
    var conn = new conexionSF();
    if(document.querySelector('input[name="certificadoFilingProbe"]:checked')){ var certificado = document.querySelector('input[name="certificadoFilingProbe"]:checked').value }else{ var certificado = " "}
    if(document.querySelector('input[name="ingresosFilingProbe"]:checked')){ var ingresos = document.querySelector('input[name="ingresosFilingProbe"]:checked').value }else{ var ingresos = " "}
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        const ret = await conn.sobject("Script_Case__c").update({
            Id: document.getElementById("scriptCaseId").value,
...
function dependentsDivProbe(){
    var conn = new conexionSF();
    if(document.querySelector('input[name="changeDependentProbe"]:checked')){ var change = document.querySelector('input[name="changeDependentProbe"]:checked').value }else{ var change = " "}
    if(document.querySelector('input[name="motivoRetirarProbe"]:checked')){ var motivo = document.querySelector('input[name="motivoRetirarProbe"]:checked').value }else{ var motivo = " "}
    if(document.querySelector('input[name="trabajaIncomeDependentProbe"]:checked')){ var trabaja = document.querySelector('input[name="trabajaIncomeDependentProbe"]:checked').value }else{ var trabaja = " "}
    if(document.querySelector('input[name="asistirIncomeDependentProbe"]:checked')){ var asistir = document.querySelector('input[name="asistirIncomeDependentProbe"]:checked').value }else{ var asistir = " "}
    if(document.querySelector('input[name="extraDeductionDependentProbe"]:checked')){ var extra = document.querySelector('input[name="extraDeductionDependentProbe"]:checked').value }else{ var extra = " "}
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        const ret = await conn.sobject("Script_Case__c").update({
            Id: document.getElementById("scriptCaseId").value,
...
function incomeDivProbe(){
    var conn = new conexionSF();
    if(document.querySelector('input[name="normasIncomeProbe"]:checked')){ var normas = document.querySelector('input[name="normasIncomeProbe"]:checked').value }else{ var normas = " "}
    if(document.querySelector('input[name="declaraIncomeProbe"]:checked')){ var declara = document.querySelector('input[name="declaraIncomeProbe"]:checked').value }else{ var declara = " "}
    if(document.querySelector('input[name="crpIncomeProbe"]:checked')){ var crp = document.querySelector('input[name="crpIncomeProbe"]:checked').value }else{ var crp = " "}
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        const ret = await conn.sobject("Script_Case__c").update({
            Id: document.getElementById("scriptCaseId").value,
...
function agregarEvento(){
    ...
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        ...
        if(bandera == true && bandera2 == true){
            const eventoCalendario = await conn.sobject("Calendar__c").create(
                {
            ...
            const evento = await conn.sobject("Event").create(
                {
                    //AccountId: document.getElementById("idAccount").value,
                    Contact__c: contact,
                    CustomerService__c: document.getElementById("csList").value,
...
function updateEvento(){
    ...
    conn.login('eautomationdep@francistaxservice.com', 'DashFLTowe16',async function(err, res) {
        if(bandera == true && bandera2 == true){
            ...
            const eventoCalendario = await conn.sobject("Calendar__c").update({   
            ...
            const evento = await conn.sobject("Event").update({   
